import numpy as np
from policies._policy_klucb import _computeKLUCB, _computeKLLCB
...
class Rank1ElimKL:
    ...
    def update(self, t, action, r):
        ...
        if self.budget == 0:
            ...
            lcbr = _computeKLLCB(t,np.sum(self.row_reward[ndx, :], axis = 1), np.sum(self.row_pulls[ndx, :], axis = 1),10)
            ucbr = _computeKLUCB(t,np.sum(self.row_reward[ndx, :], axis = 1), np.sum(self.row_pulls[ndx, :], axis = 1),10)
            ...
            lcbc = _computeKLLCB(t,np.sum(self.col_reward[:, ndx], axis = 0), np.sum(self.col_pulls[:, ndx], axis = 0),10)
            ucbc = _computeKLUCB(t,np.sum(self.col_reward[:, ndx], axis = 0), np.sum(self.col_pulls[:, ndx], axis = 0),10)
            ...
            self.budget = np.ceil(16 * np.log(self.T) / (self.gap ** 2)) - \
                np.ceil(16 * np.log(self.T) / (old_gap ** 2))
            self.bandit = self.EXPLORE_ROWS
            self.active = 0
            self.i = self.active_rows[self.active]
            self.j = self.col_defeat[np.random.randint(0, L)]
